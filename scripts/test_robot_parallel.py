...
def main():
    ...
    while logger.num_steps < max_episode:
        ...
        rospy.sleep(max(0.9 - t_move_reward, 0.05))
        ...
        if not no_bar:
            ...
            description = 'Grasp{}: r_avg={:.03f}; r={:}; eps={:.01f}; ' \
                ...
                logger.num_steps, np.array(logger.rewards).mean(),
        ...
        # print('SGD finished')
        # print('--------finished step', logger.num_steps + 1, '---------')
    envs.ur5.moveToHome()
    # saveModelAndInfo(logger, agent)
        ...
        while True:
            ...
            if render and not reward:
                plot_action(obs, agent, self.actions_star, self.actions_star_idx, self.q_value_maps, num_rotations,
                            patch_size, None, self.in_hand_obs, action_sequence, logger=logger)
