...
from utils.parameters import action_mask, model_predict_width
...
class DQN3DFCNSingleIn(DQN3DFCN):
    ...
    def forwardFCN(self, states, in_hand, obs, target_net=False, to_cpu=False):
        ...
        q_value_maps = q_value_maps.reshape((states.shape[0], self.num_primitives,
                                             self.num_rz * (1 + model_predict_width),
                                             q_value_maps.size(-2), q_value_maps.size(-1)))
