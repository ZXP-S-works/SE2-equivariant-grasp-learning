...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1, is_asr=False):
    """
    ...
    """
    if is_asr:
        local_max = np.transpose(np.nonzero(ang_img))
        q_at_nonzero = q_img[ang_img != 0]
        q_at_nonzero = q_at_nonzero.argsort()[::-1]
        local_max = local_max[q_at_nonzero[:no_grasps]]
    else:
