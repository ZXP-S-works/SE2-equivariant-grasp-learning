...
class PrioritizedQLearningBuffer:
    ...
    def sample(self, batch_size, beta, onpolicydata=False, onlyfailure=False):
        '''
        ...
        if onpolicydata and ((onlyfailure and self.buffer._storage[-1].reward.item() == 0) or not onlyfailure):
            # TODO: talk to Dian make sure the way I omit weight is correct
            onpolicyindex = npr.randint(1, onpolicy_data_aug_n + 1)
            idxes[-1] = self.__len__() - onpolicyindex
